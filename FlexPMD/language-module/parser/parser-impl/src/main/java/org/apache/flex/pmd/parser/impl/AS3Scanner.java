...
public class AS3Scanner {
    ...
    private Token scanDecimal(final char currentCharacter) {
        ...
        skipChars(result.getText().length() - 1);
        return result;
    }
    ...
    private Token scanXML() {
        ...
        for (; ; ) {
            ...
            do {
                ...
                buffer.append(currentToken.getText());
                if (isProcessingInstruction(currentToken.getText())) {
                    currentCharacter = nextChar();
                    if (currentCharacter == '\n') {
                        buffer.append('\n');
                        skipChar();
            ...
            if (currentToken.getText().startsWith("</")) {
                level--;
            } else if (!currentToken.getText().endsWith("/>")
                    && !currentToken.getText().equals("<>")) // NOT operator in AS2
            {
                level++;
            }
